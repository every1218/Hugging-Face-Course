...
학습 곡선과 훈련 분석에 대한 이해를 테스트해보십시오.

1. **훈련 손실은 감소하지만 검증 손실이 증가하기 시작할 때 일반적으로 무엇을 의미합니까?**
    1. 모델이 성공적으로 학습하고 있으며 계속 향상될 것입니다.
    2. 모델이 훈련 데이터에 과적합되고 있습니다.
    3. 학습률이 너무 낮습니다.
    4. 데이터 세트가 너무 작습니다.

2. **정확도 곡선이 부드러운 증가보다는 “계단식” 또는 고원과 같은 패턴을 보이는 이유는 무엇입니까?**
    1. 정확도 계산에 오류가 있습니다.
    2. 정확도는 예측이 결정 경계를 넘을 때만 변하는 이산적인 지표입니다.
    3. 모델이 효과적으로 학습하지 않고 있습니다.
    4. 배치 크기가 너무 작습니다.

3. **불안정하고 심하게 변동하는 학습 곡선을 관찰할 때 가장 좋은 접근법은 무엇입니까?**
    1. 수렴을 가속화하기 위해 학습률을 증가시킵니다.
    2. 학습률을 낮추고 가능하면 배치 크기를 증가시킵니다.
    3. 모델이 개선되지 않을 것이므로 즉시 훈련을 중단합니다.
    4. 완전히 다른 모델 아키텍처로 바꿉니다.

4. **언제 조기 중단 사용을 고려해야 합니까?**
    1. 항상, 어떤 형태의 과적합도 방지하기 때문입니다.
    2. 검증 성능이 개선을 멈추거나 악화되기 시작할 때입니다.
    3. 훈련 손실이 여전히 빠르게 감소하고 있을 때만입니다.
    4. 절대로, 모델이 완전한 잠재력에 도달하는 것을 방해하기 때문입니다.

5. **모델이 과소적합일 수 있음을 나타내는 것은 무엇입까?**
    1. 훈련 정확도가 검증 정확도보다 훨씬 높습니다.
    2. 훈련과 검증 성능이 모두 좋지 않고 조기에 고원에 도달합니다.
    3. 학습 곡선이 변동 없이 매우 부드럽습니다.
    4. 검증 손실이 훈련 손실보다 빠르게 감소합니다.
