...
🤗 Transformers는 `Trainer` 클래스를 제공합니다. 이 클래스를 사용하면 사전 학습된 모델을 사용자 데이터셋에 맞춰 최신 기법으로 쉽게 미세 조정할 수 있습니다. 이전 섹션에서 데이터 전처리 작업을 모두 마쳤다면 이제 몇 단계만 거치면 `Trainer`를 정의할 수 있습니다. 가장 어려운 부분은 `Trainer.train()`을 실행할 환경을 준비하는 과정일 수 있습니다. 이 작업은 CPU에서 매우 느리게 실행되기 때문입니다. 만약 GPU가 없다면 Google Colab에서 무료로 제공하는 GPU나 TPU를 이용할 수 있습니다.

📚 **훈련 리소스**: 훈련을 시작하기 전에 포괄적인 [🤗 Transformers 훈련 가이드](https://huggingface.co/docs/transformers/main/en/training)를 숙지하고 [미세 조정 쿡북](https://huggingface.co/learn/cookbook/en/fine_tuning_code_llm_on_single_gpu)의 실용적인 예제를 살펴보십시오.
...
훈련 중에 모델을 Hub에 자동으로 업로드하려면 `TrainingArguments`에서 `push_to_hub=True`를 전달하십시오. 이 기능에 대해서는 [Chapter 4](/course/chapter4/3)에서 자세히 알아보겠습니다.

🚀 **고급 설정**: 사용 가능한 모든 훈련 인수와 최적화 전략에 대한 자세한 정보는 [TrainingArguments 문서](https://huggingface.co/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments)와 [훈련 구성 쿡북](https://huggingface.co/learn/cookbook/en/fine_tuning_code_llm_on_single_gpu)을 참고하십시오.
...
📖 **더 자세히 알아보기**: Trainer 클래스와 그 매개변수에 대한 자세한 내용은 [Trainer API 문서](https://huggingface.co/docs/transformers/main/en/main_classes/trainer)를 방문하고 [훈련 쿡북 레시피](https://huggingface.co/learn/cookbook/en/fine_tuning_code_llm_on_single_gpu)에서 고급 사용 패턴을 살펴보십시오.
...
다양한 평가 메트릭과 전략에 대해 알아보려면 [🤗 Evaluate 문서](https://huggingface.co/docs/evaluate/)를 참고하십시오.
...
**혼합 정밀도 훈련**: 더 빠른 훈련과 메모리 사용량 감소를 위해 훈련 인수에서 `fp16=True`를 설정하십시오.
...
🎯 **성능 최적화**: 분산 훈련, 메모리 최적화, 하드웨어별 최적화를 포함한 고급 훈련 기술에 대해서는 [🤗 Transformers 성능 가이드](https://huggingface.co/docs/transformers/main/en/performance)를 살펴보십시오.
...
📝 **더 많은 예제**: [🤗 Transformers 노트북](https://huggingface.co/docs/transformers/main/en/notebooks)에 있는 방대한 자료를 확인해 보십시오.

## 섹션 퀴즈

Trainer API와 미세 조정 개념에 대한 이해를 테스트해보십시오.

1. **`Trainer`에서 `processing_class` 매개변수의 목적은 무엇입니까?**
    1. 사용할 모델 아키텍처를 지정합니다.
    2. 데이터 처리에 사용할 토크나이저를 Trainer에 알려줍니다.
    3. 훈련을 위한 배치 크기를 결정합니다.
    4. 평가 빈도를 제어합니다.

2. **훈련 중 평가가 얼마나 자주 발생하는지를 제어하는 TrainingArguments 매개변수는 무엇입니까?**
    1. `eval_frequency`
    2. `eval_strategy`
    3. `evaluation_steps`
    4. `do_eval`

3. **TrainingArguments에서 `fp16=True`는 무엇을 활성화합니까?**
    1. 더 빠른 훈련을 위한 16비트 정수 정밀도
    2. 더 빠른 훈련과 메모리 사용량 감소를 위한 16비트 부동소수점 수를 사용한 혼합 정밀도 훈련
    3. 정확히 16 에포크 동안 훈련
    4. 분산 훈련을 위한 16개 GPU 사용

4. **Trainer에서 `compute_metrics` 함수의 역할은 무엇입니까?**
    1. 훈련 중 손실을 계산합니다.
    2. 로짓을 예측으로 변환하고 정확도 및 F1과 같은 평가 메트릭을 계산합니다.
    3. 사용할 옵티마이저를 결정합니다.
    4. 훈련 데이터를 전처리합니다.

5. **Trainer에 `eval_dataset`을 제공하지 않으면 어떻게 됩니까?**
    1. 훈련이 오류와 함께 실패합니다.
    2. Trainer가 자동으로 훈련 데이터를 평가용으로 분할합니다.
    3. 훈련 중 평가 메트릭을 얻을 수 없지만 훈련은 여전히 작동합니다.
    4. 모델이 평가를 위해 훈련 데이터를 사용합니다.

6. **그레이디언트 누적이란 무엇이며 어떻게 활성화합니까?**
    1. 그레이디언트를 디스크에 저장하는 것으로, `save_gradients=True`로 활성화됩니다.
    2. 업데이트 전에 여러 배치에 걸쳐 그레이디언트를 누적하는 것으로, `gradient_accumulation_steps`로 활성화됩니다.
    3. 그레이디언트 계산을 가속화하는 것으로, `fp16`과 함께 자동으로 활성화됩니다.
    4. 그레이디언트 오버플로우를 방지하는 것으로, `gradient_clipping=True`로 활성화됩니다.
